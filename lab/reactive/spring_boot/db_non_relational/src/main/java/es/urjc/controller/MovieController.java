...
public class MovieController {
    ...
    @GetMapping("/{movieId}")
    public Mono<ResponseEntity<MovieResponse>> getMovieById(@PathVariable("movieId") String movieId) {
    ...
    @GetMapping
    public Flux<MovieResponse> getAllMoviesByPage(@RequestParam("page") Optional<Integer> pageNumber) {
    ...
    public Mono<ResponseEntity<MovieResponse>> updateRating(@PathVariable("movieId") String movieId,
                                                            @RequestBody RatingRequest ratingRequest) {
        return movieRepository.findById(movieId)
                .doOnNext(movie -> movie.setRating(ratingRequest.getValue()))
                .flatMap(movieRepository::save)
                .map(this::convertToResponse)
                .map(ResponseEntity::ok)
                .defaultIfEmpty(ResponseEntity.notFound().build());
    ...
    @Transactional
    @PutMapping("/{movieId}")
    public Mono<ResponseEntity<MovieResponse>> updateMovie(@PathVariable("movieId") String movieId,
    ...
    public Mono<ResponseEntity<Void>> deleteMovie(@PathVariable("movieId") String movieId) {
